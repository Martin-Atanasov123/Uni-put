...
//         group.forEach(key => {
//             const val = parseFloat(grades[key]) || 0;
//             if (val > maxVal) {
//                 maxVal = val;
//                 bestKey = key;
//             }
...
//         return {
//             key: bestKey,
//             value: parseFloat(grades[bestKey]) || 0,
//             others: group.filter(k => k !== bestKey && (parseFloat(grades[k]) > 0))
//         };
//     };
...
//             }
//         });
//         return [...examSlots, ...diplomaSlots]; // Първо Изпити/ДЗИ, после Диплома
//     };
...
//                         <Calculator size={40} />
//                     </div>
//                     <h1 className="text-4xl font-black tracking-tight uppercase italic underline decoration-primary">Калкулатор v2.7</h1>
//                 </div>
...
//                     <div className="bg-base-100 shadow-2xl p-8 border border-base-200 rounded-[3rem] animate-in fade-in duration-500">
//                         <div className="flex justify-between items-center mb-10">
//                             <h2 className="text-2xl font-black flex items-center gap-3 italic">
//                                 <Zap className="text-yellow-500" fill="currentColor" size={28} /> Балообразуващи оценки
//                             </h2>
//                         </div>
...
//                                 return (
//                                     <div key={slot.masterKey} className={`group p-8 rounded-[2.5rem] border-2 transition-all shadow-sm ${isDiploma ? 'bg-base-300 border-transparent opacity-90' : 'bg-base-200 border-primary/10 hover:border-primary/40'}`}>
//                                         <div className="flex justify-between items-start mb-6">
//                                             <div className="flex flex-col flex-1">
//                                                 <span className="text-[10px] font-black uppercase tracking-widest opacity-40 mb-1">
//                                                     {isDiploma ? "Оценка от Диплома" : "Изпит / ДЗИ слот"}
//                                                 </span>
//                                                 <select 
//                                                     className="select select-ghost select-xs font-black text-primary p-0 h-auto min-h-0 focus:bg-transparent text-sm"
//                                                     value={activeKey}
//                                                     onChange={(e) => handleGradeChange(e.target.value, grades[e.target.value] || "")}
//                                                 >
//                                                     {groupMembers.map(m => (
//                                                         <option key={m} value={m}>{FIELD_LABELS[m] || m}</option>
//                                                     ))}
//                                                 </select>
//                                             </div>
//                                             {!isDiploma && <ArrowDownUp size={16} className="text-primary opacity-40" />}
//                                         </div>
...
//                                             step="0.01"
//                                             placeholder="6.00"
//                                             className="bg-transparent text-5xl font-black w-full outline-none"
...
//                                                 <History size={12} /> Други налични: {others.map(k => `${grades[k]}`).join(", ")}
//                                             </div>
//                                         )}
//                                     </div>
//                                 );
...
//                         return (
//                             <div key={item.id} className="bg-base-100 p-10 rounded-[3rem] shadow-xl border border-base-200 group">
...
//                                     <span className="text-xs font-black opacity-40 italic font-mono uppercase tracking-widest">Мин. бал 2024: {item.min_ball_2024}</span>
//                                     {isPassing && <div className="badge badge-success py-3 px-4 font-black italic text-white rounded-xl shadow-md"><CheckCircle2 size={12} className="mr-1"/> Класиран</div>}
//                                 </div>
//                             </div>
//                         );
//                     })}
